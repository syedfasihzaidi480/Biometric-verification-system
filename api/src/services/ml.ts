...
export async function enrollVoice(files: Array<{ buffer: Buffer; filename: string; mimetype: string }>) {
...
export async function verifyVoice(enrollmentId: string, file: { buffer: Buffer; filename: string; mimetype: string }) {
  const form = new (await import('form-data')).default();
...
export async function checkLiveness(image: { buffer: Buffer; filename: string; mimetype: string }) {
  const form = new (await import('form-data')).default();
...
export async function verifyDocument(image: { buffer: Buffer; filename: string; mimetype: string }) {
  const form = new (await import('form-data')).default();
  form.append('image', image.buffer, { filename: image.filename, contentType: image.mimetype });
